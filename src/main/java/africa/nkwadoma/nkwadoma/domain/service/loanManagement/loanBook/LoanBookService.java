...
private void inviteTrainee (Loanee loanee) throws MeedlException {
    ...
    private List<RepaymentHistory> convertToRepaymentHistory(List<String[]> data) {
        ...
        for (String[] row : data) {
            RepaymentHistory repaymentHistory = RepaymentHistory.builder()
                    ...
                    .userIdentity(UserIdentity.builder().email(row[2].trim()).build()).build())
                    .paymentDateTime(LocalDateTime.parse(row[3].trim()))
