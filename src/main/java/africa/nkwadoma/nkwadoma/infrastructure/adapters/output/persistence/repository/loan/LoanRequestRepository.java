...
public interface LoanRequestRepository extends JpaRepository<LoanRequestEntity, String> {
        ...
        WHERE lr.status = 'NEW' AND (:programId IS NULL OR p.id = :programId)
        AND (:organizationId IS NULL OR o.id = :organizationId)
        AND u.isIdentityVerified = true
    """)
    Page<LoanRequestProjection> findAllLoanRequests(@Param("organizationId") String organizationId,
...
""")
    ...
    @Query("""
        ...
        select loanRequest
             from LoanRequestEntity loanRequest
             join LoanReferralEntity loanReferral on loanReferral.id = loanRequest.id
             join CohortLoaneeEntity cle on cle.id = loanReferral.cohortLoanee.id
             where cle.id = :id
    """)
    LoanRequestEntity findByCohortLoaneeId(@Param("id") String id);
}
