...
public interface CohortLoaneeRepository extends JpaRepository<CohortLoaneeEntity, String> {
    ...
    @Query("""
        select cohort_loanee.id as id,
                ...
                loan.id as loanId,loan_product.termsAndCondition as loanTermsAndCondition
               ...
               GROUP BY cohort_loanee.id,
                        ...
                        loaneeLoanDetail.id, loaneeLoanDetail.interestIncurred ,cohort.tuitionAmount,
                        loanee.id,cohort.id,loaneeLoanDetail.initialDeposit,loan.id,loan_product.termsAndCondition
    """)
...
""")
    ...
    @Query("""
    select cl from CohortLoaneeEntity cl
        JOIN LoaneeEntity lo on lo.id = cl.loanee.id
        JOIN CohortEntity co on co.id = cl.cohort.id

       where co.id = :cohortId and lo.id = :loaneeId            
    """)
    CohortLoaneeEntity findByCohortIdAndLoaneeId(@Param("loaneeId") String loaneeId,@Param("cohortId") String cohortId);
}
